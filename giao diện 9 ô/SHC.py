...
def solve_puzzle_hill_climbing_core(start, x, y, display_mode=None, update_display=None):
    """Hàm chung cho Hill Climbing, hỗ trợ cả hiển thị text và giao diện"""
    ...
    while not is_goal_state(current_board):
        ...
        if best_board is None:
            if display_mode == "text" and update_display:
                update_display('No improvement found, stuck in local optimum (Hill Climbing)')
            return
    ...
    if display_mode == "text" and update_display:
        ...
        update_display(f'Solution found in {len(path) - 1} steps (Hill Climbing)')
...
def solve_puzzle_hill_climbing(start, x, y, update_display):
    """Hill Climbing hiển thị qua text"""
...
def solve_puzzle_hill_climbing_giao_dien(start, x, y):
    """Hill Climbing hiển thị qua giao diện"""
